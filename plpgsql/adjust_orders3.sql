DROP FUNCTION kp__adjust_orders(distr_off_id bigint);
CREATE OR REPLACE FUNCTION kp__adjust_orders(
    ...
    comment text,
    basket bigint,
    state kp_enum_distr_order_state,
    packed boolean,
...
BEGIN
    ...
    SELECT
        ...
        distributions_orders.created_at,
        distributions_orders.distributions_offer AS d_off,
        distributions_orders.quantity,
        distributions_orders.quantity_adjusted,
        distributions_orders.comment,
        distributions_orders.basket,
        distributions_orders.state,
        distributions_orders.packed,
        distributions_orders.quantity_adjusted_locked,
        distributions_orders.order_price
    ...
    LOOP
        ...
        UPDATE valid_orders SET quantity_adjusted = valid_order.quantity WHERE valid_orders."id" = valid_order."id";
    END LOOP;
    ...
    FOR valid_order IN SELECT * FROM valid_orders
    LOOP
        IF NOT valid_order.quantity_adjusted_locked THEN
            adjusted_quantity := ROUND(valid_order.quantity * scale_factor / rounding_step_size) * rounding_step_size;
            adjusted_quantity := GREATEST(0, adjusted_quantity);
            UPDATE valid_orders SET quantity_adjusted = adjusted_quantity WHERE valid_orders."id" = valid_order."id";
        END IF;
    END LOOP;
    ...
    IF remaining_difference <> 0 THEN
        ...
        steps := FLOOR(ABS(remaining_difference) / rounding_step_size);
        ...
        FOR i IN 0..steps - 1 LOOP
            ...
            LOOP
                IF NOT valid_order.quantity_adjusted_locked THEN
                    UPDATE valid_orders SET quantity_adjusted = valid_orders.quantity_adjusted + step WHERE valid_orders."id" = valid_order."id";
                END IF;
            END LOOP;
        END LOOP;
        ...
        remaining_difference := adjusted_total_quantity - current_total_adjusted;
        IF remaining_difference <> 0 THEN
            -- Distribute the remaining difference to the first unlocked order
            FOR valid_order IN SELECT * FROM valid_orders
            LOOP
                IF NOT valid_order.quantity_adjusted_locked THEN
                    UPDATE valid_orders SET quantity_adjusted = valid_orders.quantity_adjusted + remaining_difference WHERE valid_orders."id" = valid_order."id";
                    EXIT;
                END IF;
            END LOOP;
        END IF;
    ...
    RETURN QUERY SELECT * FROM valid_orders;
END;
$$;

-- Example usage
SELECT * FROM kp__adjust_orders(11724);